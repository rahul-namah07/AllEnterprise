...
patch(Order.prototype, {
    ...
    async add_product(product, options) {
        if (this.pos.useBlackBoxBe() && product.get_price() < 0) {
            ...
            return;
        } else if (this.pos.useBlackBoxBe() && product.taxes_id.length === 0 && !product.combo_ids) {
            this.pos.env.services.popup.add(ErrorPopup, {
                'title': _t("POS error"),
            ...
            return;
        } else if (this.pos.useBlackBoxBe() && !product.combo_ids && !this.pos.taxes_by_id[product.taxes_id[0]].identification_letter) {
            this.pos.env.services.popup.add(ErrorPopup, {
                'title': _t("POS error"),
