...
registry.category("web_tour.tours").add('test_automatic_backorder_no_redirect', {test: true, steps: () => [
    {
        content: 'Make sure workcenter is available',
    ...
    {
        content: 'Confirm workcenter',
        extra_trigger: 'button.active:has(input[name="Workcenter1"])',
        trigger: 'button:contains("Confirm")',
    },
    {
    ...
    },
    {
        ...
        run: () => {
            ...
            helper.assert(records.length, 1);
            const recordTitle = records[0].querySelector(".card-title>span").innerText;
            helper.assert(recordTitle, "MOBACK-002");
        },
    }
]})
