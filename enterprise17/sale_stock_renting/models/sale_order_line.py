            ...
            for line in lines:
                if from_date <= fields.Datetime.now():
                    ...
                    if self.env.user.has_group('sale_stock_renting.group_rental_stock_picking'):
                        # If rental transfers are enabled, the rented_qty_during_period can overlaps with
                        # planed incoming/outgoing moves taken into account by the virtual_available qty
